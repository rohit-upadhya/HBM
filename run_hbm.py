...
if __name__ == "__main__":
    ...
    parser.add_option("-m","--max_len", action="store", type="int", dest="max_len", help="the maximum sentences number per document",default=108)
    ...
    for train_size in train_sizes:
        ...
        for seed in random_states:
            ...
            print(len(index_shuffle))
            total_train_shuffle = index_shuffle[:3000]

            train_shuffle = total_train_shuffle[:train_size]
            test_shuffle = index_shuffle[3000:]
            ...
            for e in tqdm(range(no_epochs)):
                ...
                for i, data in enumerate(tqdm(trainloader)):
                    ...
                    output = loss(out[0], labels)
                    writing_loss_train = open("train_loss.txt", "a+")
                    writing_loss_train.write('epoch '+str(e)+" "+str(output.item()))
                    writing_loss_train.write("\n")
                    writing_loss_train.close()
            ...
            #         torch.save(model.state_dict(),'./Feasibility/%s_%s_%s'%(seed,e,i))
                    ...
                    with torch.no_grad():
                        ...
                        for idx,data in enumerate(tqdm(trainloader)):
                            ...
                            output = loss(out[0], labels)
                            writing_loss_val = open("val_loss.txt", "a+")
                            writing_loss_val.write('epoch '+str(e)+" "+str(output.item()))
                            writing_loss_val.write("\n")
                            writing_loss_val.close()
        ...
        #                         attention_scores=attention_scores+[last_layer_attention]
                        ...
                        # writing_1 = open("classification_report.txt", "a+")
                        writing_2 = open("classification_report_2.txt", "a+")
                        writing_2.write(str(classification_report(y_eval_true,y_eval_pred,output_dict=True)))
                        # writing_1.write(str(acc))
                        # writing_1.write(str(f_score))
                        # writing_1.write(str(metrics.auc(fpr, tpr)))
                        # writing_1.write("\n")
                        writing_2.write("\n")
                        # writing_1.close()
                        writing_2.close()
                        macro_f.append(f_score)
                        writing_macro_f1 = open("macro_f.txt", "a+")
                        writing_macro_f1.write(str(f_score))
                        writing_macro_f1.write("\n")
                        writing_macro_f1.close()
